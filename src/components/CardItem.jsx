  ...
  return (
    <div className="w-3/4 mx-auto bg-white p-6 rounded-lg shadow-lg">
      ...
      <div className="flex items-center gap-4 mb-4 justify-start">
        ...
        <div className="text-sm text-gray-600">Yetkazib beruvchining omboridа</div>
      ...
      <div className="flex items-center justify-around gap-5 p-4">
        ...
        <div className="flex flex-col flex-grow">
          ...
          <div className="mt-4">
            <h3 className="text-lg font-medium text-gray-700 rounded">Размер: {selectedSize}</h3>
            <div className="flex gap-3 mt-3">
              {[54, 50, 46, 48].map((size) => (
                <button
                  key={size}
                  className={`px-4 py-2 border ${size === selectedSize ? 'bg-gray-200 border-blue-500 rounded' : 'border-gray-300'}`}
          ...
          <br />
          <div>
            <button className="px-6 py-3 w-64 bg-blue-500 text-white rounded-full hover:bg-blue-600">Savatga qo'shish</button>
            <br /><br />
            <button className="px-2 py-3 w-64 h-15 border-solid font-sans border-blue-400 border-[2px] text-blue-900 rounded-full">1 marta bosishda xarid qiling</button>
          </div>

          {/* Delivery Information */}
          <div className="mt-2 p-4 border border-gray-200 rounded-lg">
            <h2 className="font-bold text-lg mb-4">Yetkazib berish haqida ma'lumot</h2>
            <div className="flex justify-between items-start mb-4 cursor-pointer">
              <div className="flex flex-row items-center gap-2">
                <a href="# " className='hover:bg-blue-400'><FaLocationDot /></a>
                <div>
                  <h3 className="font-bold">Доставка</h3>
                  <a href='#' className="text-gray-500 hover:text-blue-400">Butun Toshkent shahri va butun Oʻzbekiston boʻylab</a>
                </div>
              </div>
            </div>
            <div className="flex justify-between items-start mb-4 cursor-pointer">
              <div className="flex flex-row items-center gap-2">
                <a href="# " className='hover:bg-blue-400'><PiCoatHangerLight /></a>
                <div>
                  <h3 className="font-bold">O'rnatish</h3>
                  <a href='#' className="text-gray-500 hover:text-blue-400">Toshkentda buyurtma berishda bepul jihozlash (kiyim va poyabzal).</a>
                </div>
              </div>
            </div>
          </div>
            ...
            popular.map((item) => (
              <div className='flex flex-col gap-1' key={item.id}>
                <div className='relative flex   p-2 overflow-hidden rounded-md h-[150px]'>
                  ...
                  <span className='absolute top-28 rounded-md  w-10 h-5  bg-orange-500 text-white flex items-center justify-center z-10 font-semibold'>{item.discount}</span>
      ...
      <div className='w-full flex items-start justify-start gap-6  '>
        ...
        <tbody>
          {product.map((item, index) => (
            <tr key={index} className="border-b border-dotted  border-spacing-8 border-gray-500">
        ...
        <div className='m-8'>
          ...
          <span className='text-blue-500 font-bold'><a href="#">Barcha brend mahsulotlari</a></span>
        </div>
      </div>
    </div>
  );
