				...
				<div className='flex items-center justify-around gap-24'>
					...
					<div>
						...
						<button className='cursor-pointer'>
							<a
								href='/'
								className='cursor-pointer px-4 py-2 text-white bg-customPurple rounded hover:bg-green-600 transition'
							>
								HOME
							</a>
						</button>
			...
			<div className='max-w-screen-lg mx-auto '>
				...
				<div className='grid grid-cols-2 sm:grid-cols-4 gap-6'>
					{categories.map((category, index) => (
						<div
							key={index}
							className='flex  items-center justify-around  gap-4 p-4 shadow-md rounded-lg bg-white'
						>
							<img
								src={category.image}
								alt={category.title}
								className='w-20 h-20  rounded-md object-contain mb-2'
							/>
